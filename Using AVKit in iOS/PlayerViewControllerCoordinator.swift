...
extension PlayerViewControllerCoordinator {
	...
	private func loadPlayerViewControllerIfNeeded() {
        //TODO: for default controls, set the default AVPlayerViewController()
		if playerViewControllerIfLoaded == nil {
            let playerViewController = AVPlayerViewController()
            playerViewController.showsPlaybackControls = false
            makePlayerControls(playerViewController)
			playerViewControllerIfLoaded = playerViewController
		}
	}

    private func makePlayerControls(_ playerViewController: AVPlayerViewController) {
        let playerControls = PlayerControls()
        playerControls.playerViewController = playerViewController
        playerControls.backgroundColor = .clear
        playerViewController.contentOverlayView?.addSubview(playerControls)
        ...
        NSLayoutConstraint.activate([
            playerControls.centerXAnchor.constraint(equalTo: contentOverlayView.centerXAnchor),
            playerControls.centerYAnchor.constraint(equalTo: contentOverlayView.centerYAnchor),
            playerControls.widthAnchor.constraint(equalTo: contentOverlayView.widthAnchor),
            playerControls.heightAnchor.constraint(equalTo: contentOverlayView.heightAnchor)
        ])
    }
